...
def filter_requirements(requirements_file, nvidia_packages):
    ...
    for line in lines:
        ...
        if package_name and package_name in nvidia_packages:
            ...
            filtered_lines.append(f"# {original_line}  # Skipped: NVIDIA provides {package_name}=={nvidia_packages[package_name]}")
            continue
        
        filtered_lines.append(original_line)
    
    # Backup original and write filtered version
    backup_file = requirements_file.with_suffix(requirements_file.suffix + '.orig')
    requirements_file.rename(backup_file)
    
    with open(requirements_file, 'w') as f:
        f.write('\n'.join(filtered_lines))
    
    print(f"Created {backup_file} (backup)")
    ...
    if 'project' in data and 'optional-dependencies' in data['project']:
        for group_name, deps in data['project']['optional-dependencies'].items():
            ...
            for dep in deps:
                package_name = extract_package_name(dep)
                if package_name and package_name in nvidia_packages:
                    skipped_packages.append(f"{dep} (NVIDIA provides {package_name}=={nvidia_packages[package_name]}) [from {group_name}]")
                    continue
                filtered_deps.append(dep)
            data['project']['optional-dependencies'][group_name] = filtered_deps
    
    # Backup original and write filtered version
    backup_file = pyproject_file.with_suffix(pyproject_file.suffix + '.orig')
